...
authCommand
  ...
  .addHelpText("after", "\nExample:\n  coolify-cli auth set sk-abc123xyz")
  .action((token: string) => {
    setToken(token);
...
authCommand
  ...
  .addHelpText("after", "\nExample:\n  coolify-cli auth show\n  coolify-cli auth show --raw")
  .action((opts: { raw?: boolean }) => {
    if (!hasToken()) {
      log.warn("No token configured. Run: coolify-cli auth set <token>");
      return;
    }
